...
function UILIB:Load(name, img, direction)
	...
	-- TAMBAHAN: Buat BlurEffect
	local blur = Instance.new("BlurEffect")
	blur.Size = 0
	blur.Parent = game:GetService("Lighting")
	blur.Name = "DrRayBlur"
	
	-- Sembunyikan MainBar dan TopBar saat pertama kali load
	parent.MainBar.Position = UDim2.new(0.23, 0, -0.612, 0)
	parent.TopBar.Position = UDim2.new(0.23, 0, -0.173, 0)
	parent.TopBar.TopBarClose.Position = UDim2.new(0.916, 0, 0.95, 0)
	parent.TopBar.TopBarClose.ImageLabel.Rotation = 180
	
	-- Set closed menjadi true karena UI dalam keadaan tertutup
	closed = true
        task.spawn(function()
	     while true do
		task.wait(0.1)
...
parent.TopBar.TopBarClose.MouseButton1Click:Connect(function()
	if closed == false then
		closed = true
		-- TAMBAHAN: Hilangkan blur
		local twBlur = twServ:Create(blur, TweenInfo.new(0.3), { Size = 0 })
		twBlur:Play()
	...
	elseif closed == true then
		closed = false
		-- TAMBAHAN: Aktifkan blur
		local twBlur = twServ:Create(blur, TweenInfo.new(0.3), { Size = 24 })
		twBlur:Play()
...
function UILIB.newTab(name, img)	
	...
	function self.newButton(name, func)
